...
#
# Hardware key metadata creation removed - headers are now managed by separate script
...
process_devices() {
    ...
    if [[ "$BACKUP_HEADERS" == "yes" && $headers_found -gt 0 ]]; then
        ...
        if [[ "$DRY_RUN" == "yes" ]]; then
            ...
            # Create archive with header backups only
            ...
            if [[ $? -eq 0 ]]; then
                ...
                echo "Archive includes LUKS headers"
            else
