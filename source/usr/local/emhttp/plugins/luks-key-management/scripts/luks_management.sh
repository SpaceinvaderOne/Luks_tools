...
#
create_hardware_key_metadata() {
    local metadata_file="$1"
    
    echo "Creating hardware key metadata file: $metadata_file"
    
    cat > "$metadata_file" << EOF
===============================================
LUKS Hardware-Derived Key Information
===============================================
...
===============================================
Generated by LUKS Key Management Plugin
EOF

    echo "Hardware key metadata saved to: $metadata_file"
}
...
process_devices() {
    ...
    if [[ "$BACKUP_HEADERS" == "yes" && $headers_found -gt 0 ]]; then
        ...
        local metadata_file="${HEADER_BACKUP_DIR}/hardware_key_info_${TIMESTAMP}.txt"
        
        if [[ "$DRY_RUN" == "yes" ]]; then
            echo "Dry Run: Final encrypted archive of $headers_found headers would be created at $final_backup_file"
            echo "Dry Run: Hardware key metadata would be created and included in the archive"
        else
            # Create the hardware key metadata file
            create_hardware_key_metadata "$metadata_file"
            
            echo "Creating encrypted zip archive of $headers_found headers plus hardware key metadata..."
            mkdir -p "$ZIPPED_HEADER_BACKUP_LOCATION"
            
            # Include both header backups and the metadata file in the archive
            ...
            if [[ $? -eq 0 ]]; then
                ...
                echo "Archive includes LUKS headers and hardware key information"
            else
